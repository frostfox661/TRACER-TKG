...
class RGCNLayer(nn.Module):
    def __init__(self, in_feat, out_feat, bias=None, activation=None,
        ...
        if self.skip_connect:
            self.skip_connect_weight = nn.Parameter(torch.Tensor(out_feat, out_feat))   # Cross-layer skip connection, unlike self-loop
            nn.init.xavier_uniform_(self.skip_connect_weight,
                                    gain=nn.init.calculate_gain('relu'))

            self.skip_connect_bias = nn.Parameter(torch.Tensor(out_feat))
            nn.init.zeros_(self.skip_connect_bias)  # Initialize to zero
    ...
    def forward(self, g, prev_h=[]):
        ...
        if len(prev_h) != 0 and self.skip_connect:
            skip_weight = F.sigmoid(torch.mm(prev_h, self.skip_connect_weight) + self.skip_connect_bias)     # Sigmoid to keep values in [0, 1]
            # print("skip_ weight")
            # print(skip_weight)
        ...
        self.propagate(g)  # Aggregate messages from neighboring nodes
        ...
        # print(len(prev_h))
        if len(prev_h) != 0 and self.skip_connect:   # Two loop_message paths: first activates then weights
...
class UnionRGCNLayer(nn.Module):
    ...
    def msg_func(self, edges):
        ...
        # Outgoing messages per node when it acts as head entity
...
class RGAT(nn.Module):
    ...
    def forward_v(self,g,batchsize,node,rel,tim):
        g.ndata['h'] = node
        g.edata['rel_emb'] = rel[g.edata['type']]
        # g.edata['tim_emb'] = tim[g.edata['time']]  # Can keep here or outside; block split can go here
        if self.self_loop:  # self_loop enabled
            # print(self.loop_weight)
            # loop_message = torch.mm(g.ndata['h'], self.loop_weight)
            loop_message = g.ndata['h']
            if self.dropout is not None:
                loop_message = self.dropout(loop_message)  # Zero out some entries (information dropout)
        # OOM fix idea:
        # Message propagation OOMs; later steps do not — batch g before propagation
        # Apply batching at message propagation
        # Each batch computes partial embeddings; concatenate back to full size

        # sampler =dgl.dataloading.MultiLayerFullNeighborSampler(1)
        # sampler = dgl.dataloading.MultiLayerNeighborSampler([100])
        # train_nids = np.random.choice(np.arange(g.num_nodes()), (g.num_nodes(),), replace=False)# Sample output tail nodes
        # dataloader =dgl.dataloading.NodeDataLoader\
        #     (g, train_nids, sampler, batch_size=batchsize, device=g.device)# Each block has batch_size output nodes
        ...
        self.propagate(g)  # OOM here // message propagation
        # node_repr = b
        # if self.self_loop:
        #     node_repr = node_repr + loop_message  # Sum original and propagated messages
        # g.ndata['h'] = node_repr  # Replace node features
        # quad_rel = torch.matmul(rel, self.W_r)
    ...
    def forward_v2(self,g,batchsize,node,rel,device):
        ...
        dataloader =dgl.dataloading.DataLoader\
            (g, train_nids, sampler, batch_size=batchsize, device=device)# Each block has batch_size output nodes
...
class UnionRGATLayer(nn.Module):
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        g.apply_edges(self.edge_attention) # Compute edge attention
