"""Utility functions for temporal knowledge graph link prediction."""
import numpy as np
import torch
import dgl
...
def sort_and_rank_filter(batch_a, batch_r, score, target, all_ans):
    ...
    indices = torch.nonzero(indices == target.view(-1, 1))  # indices: [B, 2]; col 0 increments, col 1 is answer entity position per row
    indices = indices[:, 1].view(-1)
    return indices
...
def slide_list(snapshots, k=1):
    ...
    :return:
    """
    k = k  # k=1: take history of length k, plus label of length 1
    if k > len(snapshots):
...
def load_data(dataset, bfs_level=3, relabel=False, data_root=None):
    ...
    if dataset in ['aifb', 'mutag', 'bgs', 'am']:
        ...
        return knwlgrh.load_link(dataset)
                     ...
                     "WIKI"]:
        return knwlgrh.load_from_local(root, dataset)
    else:
